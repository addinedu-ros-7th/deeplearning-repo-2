...
def send_drop_taxi():
    test = '2'
    sio.emit('target_droped',{'target_droped': test})
    print('하차 테스트 !!!')
...
def check_hacha():
    playsound("./data/check_hacha.wav")
    
def no_hacha():
    playsound("./data/no_hacha.wav")
    
def yes_hacha():
    playsound("./data/yes_hacha.wav")
...
def predict_with_temperature_adjustment(text, threshold=0.7, gap_threshold=0.3, temperature=1.0):
    ...
    try:
        ...
        if is_robot_called:
            ...
            if predicted_label == 1 and confidence >= threshold and is_hacha_called == False:
            ...
            elif predicted_label == 4 and confidence >= threshold and is_hacha_called == False:
                print("목적지를 새로 입력하세요")
                retake_des()
            
            elif predicted_label == 5 and confidence >= threshold and is_hacha_called == False:
                print("목적지가 확정 되었습니다")
                go_des()
            ...
            #로봇이 호출되어있는 상태에서 하차 
            elif predicted_label == 2 and confidence >= threshold and is_hacha_called == False:
                subject2 = extract_subject_hacha(text)
                target = subject2
                is_hacha_called = True
                check_hacha()
                
            elif predicted_label == 4 and confidence >= threshold and is_hacha_called == True:
                no_hacha()
                is_robot_called = False
                is_hacha_called = False
            elif predicted_label == 5 and confidence >= threshold and is_hacha_called == True:
                yes_hacha()
                # sio.emit('target_droped',{'target_droped': 'test'})
                send_drop_taxi()
                is_robot_called = False
